    ...
    class MainWindow(QMainWindow):
        def __init__(self, steampath=None):
            ...
            self.SubmitButton.resize(
                self.SubmitButton.sizeHint().width(),
                self.SubmitButton.sizeHint().height(),
            )
            self.SubmitButton.pressed.connect(self.ContinueAction)

            bottom_layout.addWidget(
                QLabel("<small>Developed by Cornelius Rosenaa</small>")
            )
        ...
        def WrongPathDialog2(self, path):  # GUI Backend
            if Util.IsWritable(path):
                ...
                PathWrong3DialogLayout.addWidget(
                    QLabel(
                        f"Please try to take ownership of this folder and try again."
                    )
                )
                ReturnButton = QPushButton(text="Return")
        ...
        def SteamDialog(self):  # GUI
            ...
            labelbox1.addWidget(
                QLabel(
                    "<p>To authorize your identity Steam has sent a code to your</p>"
                )
            )
        ...
        def SteamGuideDialog(self, parent):  # GUI
            if not self.shown:
                ...
                GuideLabel3 = QLabel(
                    "<p>Click on the shield icon in the bottom bar</p>"
                )
                GuideLabel4 = QLabel('<p>Then "Show Steam Guard code".</p>')
                GuideLabel5 = QLabel(
                    "<p>When prompted about a security warning simply</p>"
                )
                ...
                SteamConfirm = QPixmap(
                    Util.resource_path("img/SteamConfirm.png")
                ).scaled(350, 275)
        ...
        def SteamGUideCrement(self, Unit):  # GUI Backend
            if Unit == "1":
                ...
                if self.SGDIndex >= 3:
                    ...
                    self.SGDBBtn.setIcon(
                        QIcon(Util.resource_path("img/arrow-left.svg"))
                    )
            elif Unit == "-1":
                ...
                if self.SGDIndex <= 1:
                    ...
                    self.SGDFBtn.setIcon(
                        QIcon(Util.resource_path("img/arrow-right.svg"))
                    )
        ...
        def OpenStorageDialog(self):  # GUI
            ...
            StorageDialogLayout.addRow(
                QLabel(
                    "<p>(It does not matter if it's the one with Fallout 4 on it)</p>"
                )
            )
        ...
        def OpenRateDialog(self):  # GUI
            ...
            RateDialogLayout.addRow(
                QLabel("<p>If you have not logged in repeatedly</p>")
            )
            LinkLabel = QLabel(
        ...
        def DownloadDepotList(self):
            ...
            with urllib.request.urlopen(url) as dl_file:
                with open(
                    f"{self.SteamPath}/SteamFiles/DepotsList.txt", "wb"
                ) as out_file:
                    out_file.write(dl_file.read())
